function preload() {
  ...
  function create() {
    ...
    // pellet, platforms interaction 
    this.physics.add.collider(pellets, platforms, (pellet) => {
      pellet.destroy();
    })
    // pellet, player interaction
    this.physics.add.collider(pellets, gameState.player, () => {
      gameState.active = false;
      gameState.enemyVelocity = 1;
      gameState.pelletsLoop.destroy();
      this.physics.pause();
      this.add.text(110, 250, 'Game Over! Click to restart', { fontSize: '15px', fill: '#000' });
    })
    // Codey fire back
  ...
  function update() {
      ...
      if (numOfTotalEnemies() === 0) {
        ...
        gameState.enemyVelocity = 1;
        this.physics.pause();
        this.add.text(185, 250, "You Win!", { fontSize: '15px', fill: '#000000' });
      } else {
